...
void eval(char *cmdline) 
{
    ...
    // set mask to block & unblock SIGCHLD
    sigset_t mask;

    if(sigemptyset(&mask) < 0)
    {
        unix_error("eval : sigemptyset failed\n");
    }
    if(sigaddset(&mask, SIGCHLD) < 0)
    {
        unix_error("eval : sigaddset failed\n");
    }
	...
	if(builtin_cmd(argv) == 0)
	{
        ...
        {
            unix_error("eval : sigprocmask failed\n");
        }
        // block SIGCHLD
		if((pid = fork()) == 0) 
		{
		...
		else
		{
            ...
            int ground = bg ? BG : FG;
            if(addjob(jobs, pid, ground, cmdline) == 0)
            {
                app_error("eval : failed to add job\n");
            }
        ...
        //for foreground job, wait until finish
        if(!bg){
            waitfg(pid);
        }
	}
...
int builtin_cmd(char **argv) 
{
	...
	{
        // call do_bgfg function immediately
        do_bgfg(argv);
	}
    else
    {
        // if input is not built-in command, return 0
	   return 0;
    }
    return 1;
}
...
void do_bgfg(char **argv) 
{
    // if there is no second argument, print message
    if(argv[1] == NULL)
    {
    ...
    // get job with input and print message if it is not valid
    if(isPid)
    {
        pid = atoi(argv[1]);
        if(pid <= 0)
...
void waitfg(pid_t pid)
{
    ...
    // wait foreground job with busy while loop. 
    // If foreground job is done, fgjob's jid will be changed and loop will be finished.
...
void sigchld_handler(int sig) 
{
	...
	if((pid = waitpid(-1, &status, WUNTRACED | WNOHANG)) > 0)
	{
        ...
        // if child is terminated, delete it form job list
		if(WIFEXITED(status))
		{
			deletejob(jobs, pid);
        ...
        // if child is stopped, change it's status into STOP
		else if(WIFSTOPPED(status))
        {
            getjobpid(jobs, pid)->state = ST;
			printf("Job [%d] (%d) stopped by the signal %d\n", job_id, pid, WSTOPSIG(status));
		}
        // if child is terminated with signal, delete it from job list
...
void sigint_handler(int sig) 
{
    ...
    if(pid != 0)
    {
        // send SIGINT signal to all processes in foreground
        kill(-pid, SIGINT);
